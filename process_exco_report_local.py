#!/usr/bin/env python3
"""
Script to create the EXCOs workbook from the ECAG recommendation export.

Originally this script parsed multiple HTML exports. It is now simplified to
read the structured Excel export (e.g. '20251105_ECAG Reporting.xlsx') and
convert the 'Recommendations' sheet into the EXCO output workbook
('EXCO_Report_Output.xlsx') with all required sheets (ECAG open Recos,
board-member views, SII sheets, summary, dashboards, etc.).
"""

import html.parser  # still used by legacy helpers for SII template parsing
import re
...
# New primary input: structured ECAG recommendation export
RAW_EXCO_INPUT_FILE = Path(
    "/Users/satyasagardandela/Downloads/monthly Reporting/20251105_ECAG Reporting.xlsx"
)

# Legacy HTML templates are still used where necessary for styling / SII layout
...
def create_excel_workbook(df, output_file):
    ...
    # Create SII recommendation sheets. Originally this used an SII HTML template;
    # if that file is not available, derive SII recommendations directly from the
    # mapped dataframe and the original recommendations.
    print("Creating SII recommendation sheets...")
    sii_df = None
    try:
        sii_df = parse_html_table(SII_TEMPLATE_FILE)
    except Exception as exc:
        print(f"  Note: Could not load SII HTML template ({SII_TEMPLATE_FILE}): {exc}")
        sii_df = None
    ...
    if sii_df is not None and not sii_df.empty:
        ...
        # Fallback: build SII dataframe from the original recommendations dataframe
        base_df = original_df if original_df is not None else df
        if base_df is None or base_df.empty:
            sii_df = pd.DataFrame()
            sii_headers = []
        else:
            sii_df = base_df[
                base_df['Recommendation Type'].astype(str).str.contains('SII', case=False, na=False)
                | base_df['Recommendation ID'].astype(str).isin(MANUAL_SII_IDS)
            ].copy()
            sii_df = clean_dataframe(sii_df)
            sii_df = populate_board_member_flags(sii_df)
            sii_headers = list(sii_df.columns)

    if sii_df is not None and not sii_df.empty:
        # Attach manual SII rows (from original and additional sources) if any
        manual_sii_rows = pd.DataFrame()
        ...
        remaining_manual_ids = MANUAL_SII_IDS - set(manual_sii_rows['Recommendation ID'].astype(str).str.strip()) if not manual_sii_rows.empty else MANUAL_SII_IDS
        if remaining_manual_ids and additional_df is not None and not additional_df.empty:
        ...
        if manual_sii_rows is not None and not manual_sii_rows.empty:
        ...
        write_filtered_data_to_sheet(wb, sii_df, 'SII Recommendations', headers_override=sii_headers or None, template_file=FORMAT_TEMPLATE_FILE)
        ...
        if not sii_ecag_df.empty:
            write_filtered_data_to_sheet(wb, sii_ecag_df, 'SII ECAG', headers_override=sii_headers or None, template_file=FORMAT_TEMPLATE_FILE)
        ...
        for board_member, sheet_prefix in board_member_mapping.items():
            ...
            if not board_member_sii_df.empty:
                ...
                write_filtered_data_to_sheet(wb, board_member_sii_df, sheet_name, headers_override=sii_headers or None, template_file=FORMAT_TEMPLATE_FILE)
...
def main():
    ...
    print(f"Loading recommendations from Excel export: {RAW_EXCO_INPUT_FILE}")
    if not RAW_EXCO_INPUT_FILE.exists():
        print(f"Error: Input file not found: {RAW_EXCO_INPUT_FILE}")
        return

    # Load Recommendations sheet directly from the structured ECAG export
    try:
        recommendations_df = pd.read_excel(RAW_EXCO_INPUT_FILE, sheet_name='Recommendations')
    except Exception as exc:
        print(f"Error: Could not read 'Recommendations' sheet from {RAW_EXCO_INPUT_FILE}: {exc}")
        return

    if recommendations_df is None or recommendations_df.empty:
        print("Error: Recommendations sheet is empty or missing")
        return

    # Normalise headers and clean up values as before
    recommendations_df = clean_dataframe(recommendations_df)

    print(f"Found {len(recommendations_df)} recommendations in Excel export")
    print(f"Columns: {list(recommendations_df.columns)}")

    # No separate Finding Title HTML is needed because the export already contains
    # columns such as 'Finding Title' and 'Finding Description'.
    finding_titles_df = None

    print("Mapping data to EXCO template...")

    # Ensure we only use Recommendation IDs from the recommendations sheet
    ...
    # Map data using existing mapping logic
    exco_df = map_recommendations_to_exco(recommendations_df, finding_titles_df)

    # STRICT FILTER: Only keep rows where Recommendation ID exists in the recommendations sheet
